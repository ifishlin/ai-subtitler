...
and captured; what lands in assets/images/ is an ordinary image, dragged onto the frame
like any other.

    python tools/make_card.py table.html                 -> assets/images/table.png
    python tools/make_card.py table.html --name figures  -> assets/images/figures.png
...
sys.path.insert(0, str(ROOT))          # core/ lives at the root, not here
IMAGES = ROOT / "assets" / "images"
# Chrome insists on a profile directory and fills it with hundreds of files.
# That is scratch, so it belongs with the other derived things and not among
# the pictures -- where it was being committed.
SCRATCH = ROOT / "editor_cache" / "scratch"
...
def capture(page: Path, target: Path, width: int = 1400, height: int = 1200) -> Path:
    ...
    process = subprocess.Popen([
        ...
        f"--user-data-dir={SCRATCH / 'chrome-card'}",
...
def main() -> int:
    ...
    parser.add_argument("--out", default=str(IMAGES), help="輸出目錄，預設 assets/images/")
