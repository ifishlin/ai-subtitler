...
/* 順序照實際的先後，不照當初加進來的先後。短片那一套用得多，放前面；
   ...
   文件不屬於任何一組：它是 md 閱讀器，跟其他設定不是同一種東西，收進
   下拉只會更難找，留在最上層。「門」收進「設定」——那是全部文案共用的
   檢查門檻表，改的是規則本身，跟這組裡其他頁面（規定、片型、卡片……）
   同樣是「調整以後怎麼判定」，不是選一個題目或文案來看。 */

/* 整份包進一個函式裡，不留在全域——`/passages` 自己也寫了一個
   `function wire(rows, d)`（跟這支毫無關係，是剪片段那頁自己的），
   兩個都是頂層宣告，同一個名字，後載入的那個會蓋掉先載入的。結果是
   `navBar()` 呼叫的 `wire(bar)` 其實叫到了 `/passages` 那個函式，
   導覽列的下拉選單因此連一個監聽器都沒掛上，點了沒反應，也不會報錯——
   `/prompts` 也重複宣告了 `here()`，只是後果比較輕（現在頁面沒被標亮），
   同一種病。只有 `pickName`／`pickedName` 是設計成給别的頁面呼叫的，
   保留在 `window` 上；其餘的名字（`wire`／`navBar`／`link`／`here`／
   `WHERE`）都是這支檔案自己的實作細節，包起來就不會再被任何一頁的同名
   宣告蓋掉。 */
(function () {
const WHERE = [
  ["/",          "首頁",   "🏠"],
...
// 這兩個是公開的：每一頁自己的腳本要記「現在在看哪個題目／哪份文案」。
window.pickedName = pickedName;
